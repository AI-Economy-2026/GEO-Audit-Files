...
export function formatAuditContext(ctx: AuditExplanationContext): string {
...
/** Pulls the minimum audit data the LLM needs to ground its output in
 *  this brand's actual situation. Returns null if the audit is missing
 *  or not yet completed; callers fall back to a deterministic template.
 *  Shared by the explanations and overview-insights routes. */
